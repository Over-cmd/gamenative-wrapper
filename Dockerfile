...
# 1. Instalar herramientas de volcado base nativas de la imagen
RUN apt-get update && apt-get install -y --no-install-recommends ninja-build xxd git pkg-config || true

# 2. Configurar perfiles cruzados independientes para Meson (64 y 32 bits) apuntando al Sysroot real del NDK
RUN NDK_DIR=$(find /home/builder/lib -maxdepth 2 -name "android-ndk*" 2>/dev/null | head -n 1) && \
    ...
    \
    # PERFIL: 32 Bits (ARMv7) - Inyección preventiva de flags para libadrenotools
...
# 3. Script de orquestación híbrido unificado (Fusiona la capa multilib de 32/64 bits en caliente)
