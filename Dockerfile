...
# 3. Descargar, compilar e inyectar LIBDRM en el Sysroot del NDK de Android (Tanto para 32 como 64 bits)
RUN NDK_DIR=$(find /home/builder/lib -maxdepth 2 -name "android-ndk*" 2>/dev/null | head -n 1) && \
    SYS_DIR="${NDK_DIR}/toolchains/llvm/prebuilt/linux-x86_64/sysroot" && \
...
# 4. Script de orquestación híbrido unificado (Corrección del bug de buffer string y parche de memfd)
