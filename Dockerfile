...
# 2. Descargar e instalar Sysroots de Termux aislados (32 y 64 bits) sin errores de redirección
RUN mkdir -p /tmp/sysroot_64 /tmp/sysroot_32 /termux_64 /termux_32 /data/data/com.termux/files && \
    REPO="https://termux.dev" && \
    PKGS="libdrm libandroid-shmem libxcb libx11 libxshmfence libxext libxrandr libxrender xorgproto libxau libxdmcp" && \
    \
    # Descarga AArch64
    cd /tmp/sysroot_64 && curl -sL "${REPO}/dists/stable/main/binary-aarch64/Packages" > Packages && \
    for p in $PKGS; do path=$(awk -v pkg="Package: $p" '$0==pkg{f=1} f && /^Filename:/{print $2; exit}' Packages) && curl -sL -O "${REPO}/${path}"; done && \
    dpkg-deb -x *.deb /termux_64 && ln -s /termux_64/data/data/com.termux/files/usr /data/data/com.termux/files/usr64 && \
    \
    # Descarga ARM 32-bits
    cd /tmp/sysroot_32 && curl -sL "${REPO}/dists/stable/main/binary-arm/Packages" > Packages && \
    for p in $PKGS; do path=$(awk -v pkg="Package: $p" '$0==pkg{f=1} f && /^Filename:/{print $2; exit}' Packages) && curl -sL -O "${REPO}/${path}"; done && \
...
# 4. Script de orquestación híbrido (Lógica secuencial compactada para evitar desbordamiento)
