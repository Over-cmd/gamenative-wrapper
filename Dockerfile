...
# 1. Herramientas base y dependencias de Python
RUN apt-get update && \
...
# 2. Descargar Sysroots aislados de Termux (32 y 64 bits)
RUN TERMUX_REPO="https://termux.dev" && \
    ...
    mkdir -p /tmp/sysroot_64 /tmp/sysroot_32 /termux_64 /termux_32 /data/data/com.termux/files && \
    \
    cd /tmp/sysroot_64 && curl -s "${TERMUX_REPO}/dists/stable/main/binary-aarch64/Packages" > Packages && \
    for p in $PACKAGES; do path=$(awk -v pkg="Package: $p" '$0==pkg{f=1} f && /^Filename:/{print $2; exit}' Packages) && curl -L -O "${TERMUX_REPO}/${path}"; done && \
    dpkg-deb -x *.deb /termux_64 && ln -s /termux_64/data/data/com.termux/files/usr /data/data/com.termux/files/usr64 && \
    \
    cd /tmp/sysroot_32 && curl -s "${TERMUX_REPO}/dists/stable/main/binary-arm/Packages" > Packages && \
    for p in $PACKAGES; do path=$(awk -v pkg="Package: $p" '$0==pkg{f=1} f && /^Filename:/{print $2; exit}' Packages) && curl -L -O "${TERMUX_REPO}/${path}"; done && \
    dpkg-deb -x *.deb /termux_32 && ln -s /termux_32/data/data/com.termux/files/usr /data/data/com.termux/files/usr32 && \
    rm -rf /tmp/sysroot_64 /tmp/sysroot_32

# 3. Configurar perfiles cruzados mediante generadores compactos
