...
# 2. Configurar perfiles cruzados independientes apuntando al Sysroot local nativo del NDK (64 y 32 bits)
RUN NDK_DIR=$(find /home/builder/lib -maxdepth 2 -name "android-ndk*" 2>/dev/null | head -n 1) && \
    ...
    printf "[binaries]\nc = '%s/aarch64-linux-android30-clang'\ncpp = '%s/aarch64-linux-android30-clang++'\nar = '%s/llvm-ar'\nstrip = '%s/llvm-strip'\npkg-config = 'pkg-config'\n[constants]\nsys_dir = '%s'\n[properties]\npkg_config_libdir = sys_dir + '/usr/lib/aarch64-linux-android/pkgconfig'\n[built-in options]\nc_args = ['-D__TERMUX__', '-D__USE_GNU', '-U__ANDROID__', '-I' + sys_dir + '/usr/include']\ncpp_args = ['-D__TERMUX__', '-D__USE_GNU', '-U__ANDROID__', '-I' + sys_dir + '/usr/include']\nc_link_args = ['-L' + sys_dir + '/usr/lib/aarch64-linux-android/30']\ncpp_link_args = ['-L' + sys_dir + '/usr/lib/aarch64-linux-android/30']\n[host_machine]\nsystem = 'android'\ncpu_family = 'aarch64'\ncpu = 'armv8-a'\nendian = 'little'\n" "$NDK_BIN" "$NDK_BIN" "$NDK_BIN" "$NDK_BIN" "$SYS_DIR" > /root/build-config/cross_aarch64.txt && \
    \
    # PERFIL: 32 Bits (ARMv7)
    printf "[binaries]\nc = '%s/armv7a-linux-android30-clang'\ncpp = '%s/armv7a-linux-android30-clang++'\nar = '%s/llvm-ar'\nstrip = '%s/llvm-strip'\npkg-config = 'pkg-config'\n[constants]\nsys_dir = '%s'\n[properties]\npkg_config_libdir = sys_dir + '/usr/lib/arm-linux-androideabi/pkgconfig'\n[built-in options]\nc_args = ['-D__TERMUX__', '-D__USE_GNU', '-U__ANDROID__', '-I' + sys_dir + '/usr/include', '-march=armv7-a', '-mfpu=neon']\ncpp_args = ['-D__TERMUX__', '-D__USE_GNU', '-U__ANDROID__', '-I' + sys_dir + '/usr/include', '-march=armv7-a', '-mfpu=neon']\nc_link_args = ['-L' + sys_dir + '/usr/lib/arm-linux-androideabi/30']\ncpp_link_args = ['-L' + sys_dir + '/usr/lib/arm-linux-androideabi/30']\n[host_machine]\nsystem = 'android'\ncpu_family = 'arm'\ncpu = 'armv7-a'\nendian = 'little'\n" "$NDK_BIN" "$NDK_BIN" "$NDK_BIN" "$NDK_BIN" "$SYS_DIR" > /root/build-config/cross_arm.txt

# 3. Script de orquestación híbrido (Compilación de ambas arquitecturas y bypass automático de landroid)
