...
RUN NDK_DIR=$(find /home/builder/lib -maxdepth 2 -name "android-ndk*" 2>/dev/null | head -n 1) && \
    ...
    \
    # PERFIL: 32 Bits (ARMv7) - Inyección estricta del path de libandroid.so del NDK para salvar Box86
...
# 3. Script de orquestación híbrido unificado (Compila de forma nativa e integra los binarios)
