...
# 2. Configurar perfiles cruzados independientes (Inyección forzada de macros de arquitectura en cross_arm.txt)
RUN NDK_DIR=$(find /home/builder/lib -maxdepth 2 -name "android-ndk*" 2>/dev/null | head -n 1) && \
    ...
    \
    # PERFIL: 32 Bits (ARMv7) - Forzamos -D__ANDROID__=1, -D__arm__=1 y el número de llamada sys_memfd_create (356)
...
# 3. Script de orquestación híbrido unificado
