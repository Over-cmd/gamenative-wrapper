...
echo "-> A. Lanzando compilador atómico de Docker..."
docker run --rm -v "$(pwd):/workspace" ghcr.io/leegao/mesa-wrapper-ci/wrapper-compiler:latest compilacion

echo "-> B. Compilando el driver físico real de Panfrost de 64 bits de respaldo..."
...
echo "-> C. Maquetando empaque compatible ICD plano para Bannerlator..."
rm -rf pkg && mkdir -p pkg/usr/lib pkg/usr/share/vulkan/icd.d

# Mover el binario oficial nacido del Docker con el parche de memoria biónico inyectado
...
echo "🟢 PROCESO REORDENADO DE ÉXITO COMPLETADO"
